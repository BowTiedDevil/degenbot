...
- The workspace had a set of publishable crates; five of them were AMM invariant-math ports: `degenbot-v2-math`, `degenbot-concentrated-liquidity-math`, `degenbot-curve-math`, `degenbot-balancer-math`, `degenbot-solidly-math`.
...
3. **Partial (module-level) hoists stay**, plus the whole-crate alias alongside: `degenbot::core` (alias) with `address_utils`/`errors`/`hex_utils`/`runtime`/`eip_1559` still hoisted to the root; `degenbot::bot` (alias) with `bot_core` still hoisted — the `solvers` hoist from `degenbot_bot` is **dropped** so `degenbot::solvers` names the solvers *crate*; the engine lives at `degenbot::bot::solvers::arb_engine` **[amended: hoisted to `degenbot_bot::arb_engine` / `degenbot::bot::arb_engine` by the module-organization cut]**. `degenbot::uniswap` (alias) with `dex_identity`/`v2_encoding` still hoisted. Root convenience hoists of items (BotState, PoolEntry, builders, lifecycles, `preset_for_variant`, `DexIdentity`, `ReservesAbi`, …) are unchanged.
