...
- **Curve per-block caches** — 10 individual `BoundedCache` fields consolidated into `CurveOnChainCache` (Plan 054), absorbed into `CurveStableswapPool` as `_cache_*` fields with `_get_cached_*` accessors (Plan 068), then extracted into `PerBlockCache` class with mirror-free design (Plan 077). `get_cached_virtual_price()` resolves its own dependencies inline, eliminating the former side-effect mirrors (`_base_cache_updated_value`, `_base_virtual_price_value`).
