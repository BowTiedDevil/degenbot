...
| 4 | DB path resolution (`_make_arbitrage_config`: config.toml `database.path` or `~/.local/state/degenbot/db/degenbot.db`) | REACHABLE | `degenbot_config::resolve_database_path` (`rust/crates/degenbot-config/src/resolvers.rs`; re-exported as `degenbot::config`) owns `--database` > `DEGENBOT_DB_PATH` > `<state_home>/degenbot/db/degenbot.db` (XDG state home), with `~` expanded against `HOME`; `degenbot-cli`'s global `--database` feeds it through `CliContext::database_path` (`rust/crates/degenbot-cli-core/src/context.rs`). No driver resolves the path ad hoc, and the retired `[database]` file key is deliberately not consulted (ADR-051 D8). |
