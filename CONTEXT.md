...
- **`abi`** → `degenbot.abi`. `abi_adapter.py` (483 lines, companion-over-Rust with `eth_abi` fallback for fixed-point types Rust doesn't cover) dissolves into `degenbot.abi` as `degenbot.abi.adapter` or absorbed into the package. The home carries real backend-dispatch depth, not a shallow re-export. Consumers: `contract/decoding.py`, plus `aerodrome`/`aave`/`builders/*` via `abi_adapter`.
