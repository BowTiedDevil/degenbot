...
**Deferred — the hop-shape deepening.** Open: does `ResolvedHop` dissolve behind a `trait PathHopSnapshot { simulate; mobius_shape }`? Deferred to a separate session; resumes with the constraints in ADR-015 (selectivity is irreducible; pump/solve are sequential so steady-state write-contention is near-zero and the `RwLock` is for cross-task writers; a held-guard-for-whole-solve blocks concurrent writers; C-prime lands three candidates at once). Relocate first; attempt the trait shape against the whole layer in one place, later.
