    ...
    c.bench_function("get_sqrt_ratio_at_tick(mixed)", |b| {
        b.iter(|| {
            for tick in [-100000, -1000, 0, 1000, 100000].iter() {
                black_box(get_sqrt_ratio_at_tick_internal(*tick).unwrap());
            }
        })
    });
}
...
fn roundtrip_benchmark(c: &mut Criterion) {
    c.bench_function("roundtrip_tick_price_tick", |b| {
        b.iter(|| {
            ...
            let price = get_sqrt_ratio_at_tick_internal(tick).unwrap();
            let tick_back = get_tick_at_sqrt_ratio_internal(price).unwrap();
