...
#[allow(dead_code)]
pub(crate) trait PoolStateSubscriber: Send + Sync {
...
#[allow(dead_code)]
#[derive(Clone, Debug)]
pub(crate) enum DecodedPoolEvent {
...
#[allow(dead_code)]
pub(crate) trait LogDecoder: Send + Sync {
...
impl LogDispatcher {
    ...
    #[must_use]
    pub(crate) fn with_uniswap_decoders() -> Self {
        let mut d = Self::new();
    ...
    pub(crate) fn register_decoder(&mut self, decoder: Box<dyn LogDecoder>) {
    ...
    #[allow(dead_code)]
    pub(crate) fn subscribe(&self, pool_id: u64, subscriber: Weak<dyn PoolStateSubscriber>) {
        self.subscribers
            .lock()
            .entry(pool_id)
            .or_default()
    ...
    #[allow(dead_code)]
    pub(crate) fn dispatch(&self, log: &Log, state: &Arc<parking_lot::RwLock<BotState>>) {
