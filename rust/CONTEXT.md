...
**PyPool**: A thin `PyO3` handle (exposed in Python as `PyPool` — keeps the `Py` prefix per ADR-005's full-Polars naming rule; the earlier `#[pyclass(name = "Pool")]` override was dropped) that holds `Arc<parking_lot::RwLock<Bot>>` + a `pool_id` key, sharing the same `Arc` as the owning `PyBot` (see {Polars-Inspired Three-Layer Architecture}). Property reads and calculation calls cross `PyO3` on every access (read guard) — the handle owns no state itself. Today an internal bridge type (the public pool classes `UniswapV2Pool`/etc. still wrap Python state, not `PyPool`); under ADR-005 stance-ε these classes become Python companions over `PyPool`'s `V2`/`V3`/`V4` variants at cutover time, leaving the bare noun `Pool` *unreserved* at the Python tier. `calculate_tokens_out`/`calculate_tokens_in` are the per-pool swap-math surface (mirroring Python's `calculate_tokens_out_from_tokens_in`); V3/V4 single-pool CL swap math is pending implementation (the V3 stub currently returns `U256::ZERO`). No `PyToken` counterpart — tokens have no Rust-owned state (see `PyToken`).
_Avoid_: pool handle, pool wrapper

**PyToken**: A thin `PyO3` handle (exposed in Python as `PyToken` — keeps the `Py` prefix per ADR-005; the earlier `#[pyclass(name = "Token")]` override was dropped) that holds `Arc<parking_lot::RwLock<Bot>>` + an `Address` key, sharing the same `Arc` as the owning `PyBot` (see {Polars-Inspired Three-Layer Architecture}). Reads token metadata (address, decimals, symbol, name) from `Bot.tokens`. Today an internal bridge type (`Erc20Token` still wraps Python state, not `PyToken`); under ADR-005 stance-ε `Erc20Token` becomes the Python companion over `PyToken` at cutover time, leaving the bare noun `Token` *unreserved* at the Python tier.
