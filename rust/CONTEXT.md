...
**V2PoolState**: The Rust-owned state for a Uniswap V2 pool: address, token0/token1, reserves, per-direction fee parameters, factory. Data lives in `Bot.pools`, not in a Python object; the Python `UniswapV2Pool._state_cache` is a thin handle over `PyLiquidityPool`.
