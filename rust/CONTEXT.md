...
**BotCore**: The single owner of runtime pool and token state — V2 reserves/fees, V3/V4 sqrt_price/liquidity/tick/tick_data, token metadata (address, decimals, symbol, name), and reorg-rollback journals. A **peer module** to `UniswapEngine` (ADR-003): the engine holds `Arc<Mutex<BotCore>>` and reads/writes state through it, never through private per-engine stores. Lock order when nested is engine-then-core; Python-facing `PyBotCore` methods take core alone and never call into the engine (the rule that keeps the deadlock surface empty). All data lives in Rust `HashMap`s; Python holds thin `PyO3` handles (`PyPool` carrying a `pool_id` key, `PyToken` carrying an `Address` key). Owns the per-pool swap math (`calculate_tokens_out`/`calculate_tokens_in`, mirroring Python's `calculate_tokens_out_from_tokens_in`) and per-pool swap encoding (`encode_swap`); multi-pool math (path optimization) does not belong here. **Implementation status (Plan 100, Slice 1):** V2 state lives here on the live path (`register_v2_pool`, `apply_v2_sync`, `get_v2_pool_state`, `restore_all_pools_before_block`); V3/V4 state still lives on the per-family block engines until Slices 2/3 migrate them into `LiquidityMap`s. `LiquidityMap` itself does not yet exist (introduced in Slice 2 for V3).
...
**Reorg Journal**: The rollback mechanism on `BotCore`'s `PoolEntry` variants — a bounded `VecDeque` of per-block **deltas** carrying "before" values, enabling `restore_before_block(block)` to pop deltas at/after the target block and restore prior state. `V2BlockDelta` carries scalar reserve priors (cheap — two `U256`s); `V3BlockDelta` carries scalar priors (`sqrt_price_x96`, `liquidity`, `tick`) plus per-tick priors for ticks modified by Mint/Burn (Mint/Burn mutate `tick_data`; Swaps mutate only scalars, so tick priors are empty on swap deltas). `restore_before_block` also handles the "first event at the reorg target block" case (single delta at target → restore to registration state, no panic); `ReorgJournal::newest_block` gates per-pool idempotent restore in `BotCore::restore_all_pools_before_block`. **Implementation status (Plan 100, Slice 1):** V2 reorg is LIVE — `apply_v2_sync` journals deltas, the pump reads `log.removed` and calls `engine.handle_reorg(block)` (ADR-003 Option α), which restores all V2 pools + invalidates resolved paths so the next `solve_dirty` re-derives and `send_result_batch` emits `expired` diffs. V3/V4 state still lives on the per-family block engines (no journal on the live path) until Slices 2/3 migrate them into `BotCore`'s `LiquidityMap`; `V4BlockDelta` does not yet exist. Depth is user-configurable, default 1 mainnet epoch (32 blocks); reorgs deeper than the depth fail-stop the pump with a diagnostic rather than silently corrupting state.
