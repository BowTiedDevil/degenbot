...
**V2PoolState**: The Rust-owned state for a Uniswap V2 pool: address, token0/token1, reserves, per-direction fee parameters, factory. Replaces Python's `UniswapV2Pool._state_cache` — data lives in `Bot.pools`, not in a Python object.
...
**PyBot** (formerly `PyBotCore`): The `PyO3` wrapper class (exposed as `PyBot` in Python) holding `Arc<parking_lot::RwLock<Bot>>` — the Polars-style middle layer between the pure-Rust `Bot` and the Python `Bot` session. The Python `Bot.__init__` constructs one (`self._py_bot = PyBot()`); `PyPool`/`PyToken` clone the same `Arc`, so multiple Python handles share one Rust-owned `Bot` thread-safely. Provides `register_v2_pool`, `register_v3_pool`, `update_v2_pool`, `update_v3_pool`, `calculate_tokens_out`, `calculate_tokens_in`, `encode_swap`, `get_pool`, `register_token`, `get_token` — queries/calcs take a read guard, mutations a write guard. Thin `PyPool`/`PyToken` handles returned by `get_pool`/`get_token` are the Rust-core read entry points. `register_v3_pool` / `register_v4_pool` register the pool with `Bot` and apply staged backfill/pump buffers (`apply_backfill_buffer` → `apply_pump_buffer`); on-chain verification (`liquidity_verifier::verify_v3_pool` / `verify_v4_pool`) is invoked ad-hoc from `py_binding.rs` (the free-function surface, not a method on a `LiquidityMap` — the generic was never extracted; see {LiquidityMap}).
_Avoid_: pybot wrapper, python bot handle

**PyPool**: A thin `PyO3` handle (exposed as `Pool` in Python) that holds `Arc<parking_lot::RwLock<Bot>>` + a `pool_id` key, sharing the same `Arc` as the owning `PyBot`. Property reads and calculation calls cross `PyO3` on every access (read guard) — the handle owns no state itself. Under ADR-003 `PyPool` is the Rust-core construction entry point (the Python `Bot.build_pool` eventually returns one instead of a Python `AbstractLiquidityPool`). `calculate_tokens_out`/`calculate_tokens_in` are the per-pool swap-math surface (mirroring Python's `calculate_tokens_out_from_tokens_in`); V3/V4 single-pool CL swap math is pending implementation (the V3 stub currently returns `U256::ZERO`). No `PyToken` counterpart — tokens have no Rust-owned state (see `PyToken`).
_Avoid_: pool handle, pool wrapper

**PyToken**: A thin `PyO3` handle (exposed as `Token` in Python) that holds `Arc<parking_lot::RwLock<Bot>>` + an `Address` key, sharing the same `Arc` as the owning `PyBot`. Reads token metadata (address, decimals, symbol, name) from `Bot.tokens` — the Rust-core analog of pool-side `PyPool` over state. Python's `Erc20Token` wraps a `PyToken` and adds the I/O-layer concerns that could not move to Rust (price oracle, display helpers): the split tracks the same line as `PyPool` (Rust owns state+math) vs. `PyBot`/orchestration (Python owns I/O).
...
- **`PyPool`** → **`Bot`**: The thin handle holds `Arc<parking_lot::RwLock<Bot>>` + `pool_id` (shared with the owning `PyBot`); `calculate_tokens_out/in` delegate directly to `Bot` via the key (read guard)
- **`PyToken`** → **`Bot`**: The thin handle holds `Arc<parking_lot::RwLock<Bot>>` + `Address` (shared with the owning `PyBot`); future property reads (symbol, decimals) will delegate through the same pattern (read guard)
