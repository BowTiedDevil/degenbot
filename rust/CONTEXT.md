...
**degenbot-decoders**: The pure-Rust workspace crate (Plan 104) holding the Uniswap V2/V3/V4 event-log **decoders** — `rust/crates/degenbot-decoders/src/`. An **alloy-only leaf** (no `pyo3`/`tokio`/`degenbot-core`/`degenbot-abi`): each decoder hand-slices EVM log bytes and returns a plain `Option<Event>` struct. Exports the six topic constants (`V2_SYNC_TOPIC`, `V3_SWAP_TOPIC`, `V3_MINT_TOPIC`, `V3_BURN_TOPIC`, `V4_SWAP_TOPIC`, `V4_MODIFY_LIQUIDITY_TOPIC`), the decode fns (`decode_sync_log`/`decode_v3_swap_log`/etc.), their return structs (`SyncEvent`/`V3SwapEvent`/etc.), and the V4 `PoolId` type alias (`[u8; 32]`). The **state-coupled dispatch layer** (`LogDecoder` trait, `DecodedPoolEvent`, `LogDispatcher` bus, `PoolStateSubscriber`) stays in `degenbot-bot`'s `bot_core/log_dispatcher.rs` (touches `BotState`); it reaches the leaf decode fns via `degenbot_decoders::`. Bot-internal consumers reference the crate by its direct path (`use degenbot_decoders::...`) — the codebase convention (mirrors how `degenbot-bot` already consumes `degenbot_core`/`degenbot_abi`), NOT via a re-export shim. Extension seam: a future Curve/Aave event decoder lands here alongside the Uniswap ones.
_Avoid_: decoder crate, decoders module, log-decoder crate

**V2_SYNC_TOPIC**: `0x1c411e9a96e071241c2f21f7726b17ae89e3cab4c78be50e062b03a9fffbbad1` — the Keccak256 hash of the Uniswap V2 `Sync(uint112,uint112)` event signature. Lives in `degenbot_decoders::v2_sync_decoder`. The Rust-side constant matches the Python `V2_SYNC_TOPIC` in `log_decoders.py`.
...
**Bot**: The per-chain Rust orchestrator — owns runtime pool/token state (V2 reserves/fees, V3/V4 `sqrt_price_x96`/`liquidity`/`tick`/`tick_data`, token metadata, reorg-rollback journals), the RPC `AlloyProvider`, the `chain_id` (construction-time `Bot::new(chain_id)`), the per-block pump, and a `Vec<Box<dyn EventSink>>` of attached engines. One `Bot` = one chain + one RPC; multi-chain is N Bots. A single shared `Arc<RwLock<Bot>>` is the canonical state owner — `PyBot` holds it directly, `UniswapEngine` adopts it via `with_core` (which takes `Arc<RwLock<BotState>>`). Pool registration is a `Bot` concern; engines discover pools at `register_path` time. See ADR-006 for the per-chain-orchestrator decision and ADR-003 for the Bot-as-state-owner split.
_Avoid_: core, bot state, rust state owner

**PoolEntry**: An enum with one variant per pool family (`V2(V2PoolIdentity, V2PoolState)`, `V3(V3PoolIdentity, V3PoolState)`, `V4(V4PoolIdentity, V4PoolState)`, `Curve(CurvePoolIdentity, CurvePoolState)`, `BalancerWeighted(BalancerWeightedPoolIdentity, BalancerWeightedPoolState)`, `BalancerStable(BalancerStablePoolIdentity, BalancerStablePoolState)`). Every variant is a `(VxPoolIdentity, VxPoolState)` tuple — ADR-005 identity/state split: the immutable half holds registration identity (set once at register time), the mutable half holds slot0 scalars/balances + the reorg journal + the pinned verify seeds. The identity half is borrowed via `get_<family>_identity(pool_id)`; the mutable half via `get_<family>_pool(pool_id)`. Calculation methods match on the variant — no trait objects, no vtable dispatch.
_Avoid_: pool enum, pool type enum

**V2PoolState**: The Rust-owned state for a Uniswap V2 pool: address, token0/token1, reserves, per-direction fee parameters, factory. Data lives in `Bot.pools`, not in a Python object; the Python `LiquidityPool._state_cache` is a thin handle over `PyLiquidityPool`.
...
**Polars-Inspired Three-Layer Architecture**: The FFI topology for stateful Rust-owned resources (ADR-005, revised by ADR-006). A `#[pyclass]` wrapper (`PyBot`) holds `Arc<parking_lot::RwLock<Bot>>` and *is* the sharing mechanism; thin handles (`PyLiquidityPool`/`PyErc20Token`) clone that `Arc` so N Python objects reference one Rust-owned core; the Python `Bot` session constructs and owns the wrapper (`self._py_bot = PyBot()`). Read methods take a read guard, mutations a write guard. Grounded in Polars' `RwLock<DataFrame>` + `Arc`-shared `SharedStorage`. `Bot` is the per-chain orchestrator (owns RPC + `chain_id` + the pump + a `Vec<Box<dyn EventSink>>`); `UniswapEngine` adopts the *shared* `Arc<RwLock<Bot>>` via `with_core`; pool registration is a `Bot` concern; the engine associates with a `Bot` as an **EventSink**. Standalone-Rust-core means a Rust user can run the whole bot without Python (Polars split-UX). The stateful specialization of `rust/AGENTS.md`'s generic three-layer convention; complements ADR-003 (which answers *who owns state*, not how Python reaches it).
_Avoid_: polars model, polars-style middle layer, FFI wrapper topology

**EventSink**: The cycle-free association topology between a `Bot` (per-chain orchestrator/owner of the pump + helpers) and its engines, realized as a per-state-subject publisher/subscriber event bus — not raw-log passthrough. Two traits implement it in code: `EventSink` (`rust/crates/degenbot-bot/src/bot_core/engine.rs`) is the lifecycle attachment trait (`Bot` holds `Vec<Box<dyn EventSink>>`); `PoolStateSubscriber` (`rust/crates/degenbot-bot/src/bot_core/log_dispatcher.rs:40`) is the per-update notify trait with the single method `on_pool_state_updated(&self, pool_id: u64)`. `Bot`'s `pub(crate)` helpers — `BotState` (pure-data registries/swap math/journal/buffers), `LogDispatcher` (decoder registry + `Weak<dyn PoolStateSubscriber>` registry), `BlockPump` (`rust/crates/degenbot-bot/src/bot_core/block_pump.rs:107`, WS transport + drain loop), `SolveCoordinator` (drain-point solve tick), `ReorgCoordinator` (`rust/crates/degenbot-bot/src/bot_core/reorg_coordinator.rs:112`, restore) — stay private and never widen `Bot`'s public surface (caller uses `new`/`attach_engine`/`register_pool`/`start`). Pool-state events (Sync/Swap/Mint/Burn/ModifyLiquidity) are decoded + applied to `BotState` by `Bot` itself; after the core write lock releases, `LogDispatcher` notifies subscribers per `pool_id`. The engine dirties `pool_id` in its **own** per-engine dirty-set (taking the engine `Mutex` alone — core write already released, preserving the engine-then-core lock order) and reads `BotState` only later in `solve_dirty`. `ReorgCoordinator` handles reorg optimistically per removed event — on every `removed: true` log for pool P at block `B`, it calls `restore_before_block(B)` for P and fires the **same** `on_pool_state_updated(P)` notification as a forward update (no separate `on_reorg` method). `UniswapArbEngine` is the current sole subscriber; a future `AaveLiquidationEngine` would implement the same trait with no `Bot`/pump/`PyBot` change. See ADR-006.
_Avoid_: engine callback, block callback, sink trait (the last collides with subscription vocabulary)

**PyBot**: The `PyO3` wrapper class (exposed as `PyBot` in Python) holding `Arc<parking_lot::RwLock<Bot>>`. The Python `Bot.__init__` constructs one (`self._py_bot = PyBot()`); the FFI topology this wrapper participates in is defined in **Polars-Inspired Three-Layer Architecture** (ADR-005). Provides `register_v2_pool`, `register_v3_pool`, `update_v2_pool`, `update_v3_pool`, `calculate_tokens_out`, `calculate_tokens_in`, `encode_swap`, `get_pool`, `register_token`, `get_token`, and `unregister_pool` — queries/calcs take a read guard, mutations a write guard. `unregister_pool` is the symmetric removal half of the register seam (ADR-007): V2/V3 only on `PyBot` (V4 registration lives on `UniswapArbEngine`, so the V4 tuple-key removal belongs there too — `PyBot::unregister_pool` rejects a `pool_id` set). Removed `pool_id`s are **retired** (not reused) so a stale `PyLiquidityPool` handle cannot alias to a different pool on recreate; `next_pool_id` only ever advances. Thin `PyLiquidityPool`/`PyErc20Token` handles returned by `get_pool`/`get_token` are the Rust-core read entry points. `register_v3_pool` / `register_v4_pool` register the pool with `Bot` and apply staged backfill/pump buffers (`apply_backfill_buffer` → `apply_pump_buffer`); on-chain verification (`liquidity_verifier::verify_v3_pool` / `verify_v4_pool`) is invoked ad-hoc from `bot/engine/verify.rs` (the free-function surface, not a method on a `LiquidityMap` — the generic was never extracted; see **LiquidityMap**).
_Avoid_: pybot wrapper, python bot handle

**PyLiquidityPool**: A thin `PyO3` handle (exposed in Python as `PyLiquidityPool` — keeps the `Py` prefix per ADR-005's full-Polars naming rule; the earlier `#[pyclass(name = "Pool")]` override was dropped) that holds `Arc<parking_lot::RwLock<Bot>>` + a `pool_id` key, sharing the same `Arc` as the owning `PyBot` (see **Polars-Inspired Three-Layer Architecture**). Property reads and calculation calls cross `PyO3` on every access (read guard) — the handle owns no state itself. Generalized by design (no per-variant `PyV2PoolState`/`PyV3PoolState`/`PyV4PoolState` wrappers; the variant lives only as internal `PoolEntry` dispatch). The generalized `LiquidityPool` Python companion wraps `PyLiquidityPool` (the hollow V2 DEX subclasses `SushiswapV2Pool`/etc. are deleted; DEX identity is `DexIdentity` data passed as `dex=`, not a class hierarchy — see `docs/migration-guides/dex-subclass-collapse.md`). `calculate_tokens_out`/`calculate_tokens_in` are the per-pool swap-math surface (V2 constant-product + V3/V4 concentrated-liquidity via `v3_simulate_swap`/`v4_simulate_swap`, mirroring Python's `calculate_tokens_out_from_tokens_in`). `PyErc20Token` is the token-side counterpart.
...
**Unregister Seam (ADR-007)**: The symmetric removal half of the register seam — `BotState::unregister_pool(address, pool_id: Option<PoolId>) -> bool` + `PyBot::unregister_pool` (V2/V3 only on `PyBot`; V4 tuple-key belongs on the engine — see **PyBot**). Drops the `PoolEntry` (and its **Reorg Journal** with it), the index entries (`pool_addresses` for V2/V3; `v4_pool_ids` for V4), and drains matching `v3_buffer`/`v4_buffer` events via `LiquidityEventBuffer::discard_for` so a re-register does not replay stale Mint/Burn/ModifyLiquidity onto the fresh pool. Silent-on-miss returns `false` (mirrors Python `PoolRegistry.remove`); `register_*` stays refusal-on-`panic!`/`Err` on duplicate (ADR-007 U2) — the asymmetry reflects the operations' invariant asymmetry, not an inconsistency. `next_pool_id` is **not** reused: removed ids are retired so a stale `PyLiquidityPool` handle cannot alias to a different pool on recreate. The Python side: `PoolRegistry.__init__` takes a keyword-only `py_bot: PyBot | None = None`; `PoolRegistry.remove`/`_reset` call `py_bot.unregister_pool` for the V2/V3 path when set (when `None` — standalone test constructions — removal is Python-only). V4 removal is Python-only on the `ManagedPoolRegistry` (engine-side `unregister_v4_pool` is ADR-007 Deferred — no `PyBot::unregister_pool` path for V4 today). See ADR-007.
_Avoid_: orphan flag, reorg bit, unsubscribe flag

**Reorg Journal**: The rollback mechanism on `Bot`'s `PoolEntry` variants — a bounded `VecDeque` of per-block **deltas** carrying "before" values, enabling `restore_before_block(block)` to pop deltas at/after the target block and restore prior state. `V2BlockDelta` carries scalar reserve priors (two `U256`s); `V3BlockDelta` carries scalar priors (`sqrt_price_x96`, `liquidity`, `tick`) plus per-tick priors for ticks modified by V3 Mint/Burn or V4 `ModifyLiquidity` (the same `V3BlockDelta` shape serves both V3 and V4 CL families — no separate `V4BlockDelta`). `ReorgJournal::newest_block` gates per-pool idempotent restore in `Bot::restore_all_pools_before_block`, which dispatches V2 (scalar reserves) vs V3/V4 (`v3_restore_before_block`/`v4_restore_before_block`: scalars + reverse-applied per-tick priors + tick-range cache invalidation). `apply_v2_sync`/`apply_v3_swap`/`apply_v4_swap` journal deltas on every mutation; `ReorgCoordinator` (`rust/crates/degenbot-bot/src/bot_core/reorg_coordinator.rs:112`) handles reorg optimistically per removed event — on every `removed: true` log for pool P at block `B`, it calls `restore_before_block(B)` for P and fires the same `on_pool_state_updated(P)` notification as a forward update. Depth is user-configurable via `Bot::with_journal_depth`, default 1 mainnet epoch (32 blocks); reorgs deeper than the depth fail-stop the pump with a diagnostic rather than silently corrupting state. `BotState::unregister_pool` (ADR-007) drops the `PoolEntry` and its journal together — restore for a removed `pool_id` is a no-op target (the journal no longer exists); the journal depth and pump reorg detection are unaffected (they key off `address`/`pool_id` that no longer resolve for the removed pool and naturally skip it). See ADR-003.
_Avoid_: delta-journal, state history, rollback stack, undo log

**Slot0 Head / Tick Bookkeeping Map**: The conceptual two-part split *within* a single CL pool's state (`V3PoolState` / `V4PoolState`). The **slot0 head** = the high-churn scalar state (`sqrt_price_x96`, `tick`, active `liquidity`), mutated on *every Swap*. The **tick bookkeeping map** = `tick_data: HashMap<i32, TickInfo>` (initialized ticks' `liquidity_gross`/`liquidity_net`), mutated *only* by Mint/Burn (V3) or `ModifyLiquidity` (V4) — low churn. ADR-004's `TickMap` trait (`rust/crates/degenbot-bot/src/bot_core/tick_map.rs:41`) narrows the verifier/apply views to the tick map only — the six `takes-whole-but-wants-one` sites (`verify_v3_pool`/`verify_v4_pool` + `apply_v{3,4}_liquidity_update` + batch entry points) take `&impl TickMap`; the slot0 head is never read there ("would always be stale"). `V3BlockDelta` carries the two as separate journal sub-structures (scalar priors + per-tick priors). `v3_simulate_swap`/`v4_simulate_swap`/`build_int_*_sequence` keep taking `&V3PoolState`/`&V4PoolState` flat — the slot0+tick lockstep reads there are a genuine "both" consumer. See ADR-004 (the full-struct-split reading was rejected: no `slot0-only` reader exists).
_Avoid_: hot/cold state split, head vs. map, slot vs. ticks

**V3SwapTopic**: `0xc42079f94a6350d7e6235f29174924f928cc2ac818eb64fed8004e115fbcca67` — the Keccak256 hash of the Uniswap V3 `Swap(address,address,int256,int256,uint160,uint128,int24)` event signature. Used by the V3 swap decoder (`degenbot_decoders::v3_swap_decoder`) to filter and decode logs.
...
**UniswapEnginePump**: The unified pump that drives `UniswapEngine`. Maintains two concurrent WS subscriptions — `newHeads` (block boundary notifications) and `logs` (unfiltered, all filtering in Rust). Each WS log is applied to engine state immediately via `apply_log` (zero-latency update), then solves are deferred and coalesced at the top of the next loop iteration into a single `solve_dirty` call. Solves and sends are decoupled: `solve_dirty` updates `self.results` without sending; `send_result_batch` (→ `compute_diff_and_send`) is driven by a 50ms debounce timer (`DEBOUNCE_MS`) that force-flushes on block boundaries (`finalize_if_dirty`) and the 60s-timeout recovery path. Rust-side `filter_relevant_logs()` checks topic0 against the 6 monitored topics AND address against registered pool/PoolManager addresses — no provider-side filter needed. Spawned by `PyUniswapArbEngine::resume_from_subscribe()` (canonical flow: `subscribe` → `backfill_from_snapshot` → `resume_from_subscribe`).
_Avoid_: unified pump, combined pump, single pump, per-block getLogs pump

**V3_MINT_TOPIC**: `0x7a53080ba414158be7ec69b987b5fb7d07dee101fe85488f0853ae16239d0bde` — the Keccak256 hash of the Uniswap V3 `Mint(address,address,int24,int24,uint128,uint256,uint256)` event signature. Used by the V3 Mint/Burn decoder (`degenbot_decoders::v3_mint_burn_decoder`) to filter and decode Mint logs. Matches the Python `V3_MINT_TOPIC` in `log_decoders.py`.

**V3_BURN_TOPIC**: `0x0c396cd989a39f4459b5fa1aed6a9a8dcdbc45908acfd67e028cd568da98982c` — the Keccak256 hash of the Uniswap V3 `Burn(address,int24,int24,uint128,uint256,uint256)` event signature. Used by the V3 Mint/Burn decoder (`degenbot_decoders::v3_mint_burn_decoder`) to filter and decode Burn logs. Matches the Python `V3_BURN_TOPIC` in `log_decoders.py`.
...
**UniswapEngine**: The unified engine that composes V2/V3/V4 pool-state handling for mixed arbitrage paths in a single per-block lifecycle. A **peer module** to `Bot` (ADR-003): owns path registry, solver dispatch, result batching, the pump, and diagnostics, but owns **no pool state** — it reads/mutates state through the shared `Arc<RwLock<Bot>>` adopted via `with_core` (engine keeps its own `Mutex<UniswapEngine>` for path/solver state; lock order engine-then-core). Routes Sync events to V2 handling, Swap events to V3, V4 Swap events (from PoolManager) to V4. Intake is `register_path` resolving `pool_id`s against the associated `Bot` — the engine constructs no pools. Solver dispatch: V2-V2 → `exact_mobius_solve`; V3-V3 and V4-V4 → `int_solve_v3_v3` (same CL math); V2-V3/V3-V2/V4-V3/V3-V4/V4-V2/V2-V4 → `exact_solve_mixed_v2_v3_sequence` (integer effective reserves + piecewise tick-range enumeration). Associates with a `Bot` as an **EventSink**. The peer-module split (Bot=state, engine=paths) and engine-then-core lock order are from ADR-003; the shared-core arrangement is from ADR-006.
...
**ResultBatch**: An incremental diff between two solver runs. Carries `solve_block`, block metadata (timestamp, fees, gas), and four lists: `fresh` (paths newly within the profit window), `updated` (paths within the window with changed values), `expired` (paths that left the window), `removed` (de-registered paths). Sent to Python via an unbounded `mpsc` channel — every diff is delivered, no silent drops. The `self.delivered` HashMap in `UniswapEngine` tracks what Python has seen, enabling correct incremental diff computation on each call. _Profit window_: `compute_diff_and_send` filters with `profit > min_profit && profit <= max_profit` (strict lower, inclusive upper). `min_profit` is the lower actionability threshold (default `U256::ZERO`); `max_profit` is an upper sanity cap that excludes likely-defect / scam-token profits (default `U256::MAX`, i.e. disabled — the inclusive bound makes `U256::MAX` a true open cap). Set via `set_profit_thresholds` (Python: `min_profit` required, `max_profit` optional, both full `uint256`-scale; `None` = no cap); results outside the window are excluded from `delivered` and from batches.

_Avoid_: solver result, solve output, batch result

**Eager Processing**: The pump architecture where WS logs are applied to engine state immediately upon arrival (via `apply_log`), with solver runs deferred and coalesced at the top of the pump loop. Solve and send are decoupled: solves fire at the top of each loop iteration when `has_dirty_paths()` is true; sends are debounced via the 50ms `DEBOUNCE_MS` timer that force-flushes on block boundaries (`finalize_if_dirty`) and the 60s-timeout recovery path. Multiple logs arriving between `await` points batch into a single `solve_dirty`, and a burst of logs yields a single `send_result_batch`. See the `UniswapEnginePump` entry and `docs/architecture/rust-owned-bot.md` for the full pump loop.
...
**PyUniswapArbEngine**: The PyO3 wrapper class (exposed as `UniswapArbEngine` in Python) that holds `Arc<Mutex<UniswapEngine>>` plus a shutdown flag and pump handle. Provides `register_v2_pool`, `register_v3_pool`, `register_v4_pool`, `register_path`, `process_logs`, `latest_results`, and `backfill_from_snapshot`. The two-phase startup is `subscribe()` → `backfill_from_snapshot()` → `resume_from_subscribe()`.
...
**V4PoolKey**: A struct with 5 fields matching Solidity's `PoolKey`: `currency0: Address`, `currency1: Address`, `fee: u32`, `tick_spacing: i32`, `hooks: Address`. Used for engine-internal tracking; the hooks field is stored but not used for solving (hook filtering happens at registration).
...
**LiquidityMap**: Conceptual first-class owner of accurate CL pool state for one pool family, bundling three operations — applying events (Swap, Mint/Burn, ModifyLiquidity); buffering deferred liquidity events for unregistered pools; and verifying state against on-chain truth. Never implemented as a generic (ADR-003's no-abstraction-against-sample-of-one discipline): the live shape is the inline `PoolEntry::V3(V3PoolIdentity, V3PoolState)`/`PoolEntry::V4(V4PoolIdentity, V4PoolState)` collection in `Bot.pools`, with apply methods on `Bot` (`apply_v3_swap`/`apply_v3_liquidity_update` + V4 mirrors) and verify functions in `liquidity_verifier.rs` (`verify_v3_pool`/`verify_v4_pool`/`verify_v3_liquidity_map`/`verify_v4_liquidity_map`). ADR-004's `TickMap` trait narrows the verifier/apply views (impl'd on the `(VxPoolIdentity, VxPoolState)` tuple post ADR-005 identity/state split — so the trait can read both the immutable `address`/`tick_spacing` and the mutable `tick`/`tick_data` off one registry slot). Trait extraction deferred until a third CL-style family (Curve) ports and proves the shape. See ADR-003 + ADR-005 identity/state split.
...
**Verify-Against-Onchain**: The on-chain self-check comparing in-memory tick data (raw snapshot at the snapshot block, post-backfill state at the backfill block) against on-chain truth via RPC. Lives as the free-function surface in `liquidity_verifier.rs`: `verify_v3_liquidity_map` / `verify_v4_liquidity_map` (the snapshot-block variants, which already take a typed `&HashMap<i32, TickInfo>` + `Address` + block — the typed-boundary precedent ADR-004 generalizes) and `verify_v3_pool` / `verify_v4_pool` (the live variants, which take `&V3PoolState` / `&V4PoolState` and recover the "don't read slot0" rule from the module doc today — ADR-004's `TickMap` trait narrows these to take `&impl TickMap`). Invoked ad-hoc from `bot/engine/verify.rs`; not a method on a `LiquidityMap` (that generic was never extracted — see **LiquidityMap**).
_Avoid_: state check, registration check, verify hook

**AMOUNT_MODIFYING_HOOK_MASK**: `0xCC` — a bitmask covering the 4 V4 hook flags that can modify swap amounts: `BEFORE_SWAP` (0x80), `AFTER_SWAP` (0x40), `BEFORE_SWAP_RETURNS_DELTA` (0x08), `AFTER_SWAP_RETURNS_DELTA` (0x04). Pools with `(hook_flags & 0xCC) != 0` are rejected by the Rust core in `BotState::register_v4_pool` (a *correctness floor* — the solver's V3-CL math assumes no hook intervention; enforced here, not in Python, per ADR-005's standalone-core constraint). The refusal surfaces at the PyO3 seam as `HookedPoolRejectedError` (subclass `ValueError`).
_Avoid_: hook mask, hook filter, amount mask

**V4_DYNAMIC_FEE_FLAG**: `0x100000` — the fee value indicating a V4 pool has dynamic (swap-dependent) fees. Pools with this fee value are rejected by the Rust core in `BotState::register_v4_pool` (the solver assumes a fixed fee), surfacing as `DynamicFeePoolRejectedError` (subclass `ValueError`).
...
- **`Bot`** → **`V2PoolState`**: V2 pools store the mutable reserves and update block — `V2PoolState` (ADR-005 identity/state split); the per-direction fee parameters (`fee_token0`, `fee_token1`) and other registration config live on the immutable `V2PoolIdentity`. Eliminates the need for Python's `V2PoolState` cache object.
- **`Bot`** → **`RegisterV2PoolParams`**: The params struct bundles 9 registration fields into one argument; `bot/mod.rs` (`PyBot`) constructs it from Python keyword arguments
...
- **`PyBot`** → **`BotState::unregister_pool`** (ADR-007): `PyBot::unregister_pool(address, pool_id=None)` delegates to `BotState::unregister_pool`; the Python `PoolRegistry.remove`/`_reset` propagate through this so removal of a V2/V3 pool reaches Rust state. `LiquidityEventBuffer::discard_for(key)` (single-key inverse of `flush`) drains the `v3_buffer`/`v4_buffer` during unregister so re-register doesn't replay stale events. V4 tuple-key removal is engine-side (deferred).
...
- **`PyUniswapArbEngine`** → **`UniswapEnginePump`**: `subscribe(rpc_url)` opens + observes the pump; `backfill_from_snapshot()` closes the snapshot→WS gap; `resume_from_subscribe()` begins normal processing. `pump_handle` stores the JoinHandle; `shutdown` `AtomicBool` stops it
...
- **`UniswapEngine`** → **`V2BlockEngine`**: UniswapEngine composes a V2BlockEngine for V2 pool state and constant-product solving
...
- **`UniswapEngine`** → **`V3BlockEngine`**: UniswapEngine composes a V3BlockEngine for V3 pool state, tick ranges, and piecewise solving
- **`UniswapEngine`** → **`V4BlockEngine`**: UniswapEngine composes a V4BlockEngine for V4 pool state, tick ranges, and piecewise solving
...
- **`Bot`** owns one `HashMap<u64, PoolEntry>` with `PoolEntry::V2(V2PoolIdentity, V2PoolState)` / `V3(V3PoolIdentity, V3PoolState)` / `V4(V4PoolIdentity, V4PoolState)` (and Curve + Balancer twins, each a `(VxPoolIdentity, VxPoolState)` tuple per ADR-005 identity/state split) variants — a single inline store (the `LiquidityMap` generic was never extracted; see **LiquidityMap**). ADR-004's `TickMap` trait narrows the verifier/apply views of the V3/V4 entries; the trait impl is on the `(VxPoolIdentity, VxPoolState)` tuple so it reads identity + mutable from one slot.
...
- **`PyBot`** exposes `register_v3_pool` / `register_v4_pool` as `Bot` registration with staged buffer drain (`apply_backfill_buffer` → `apply_pump_buffer`). On-chain verification (`verify_v3_pool` / `verify_v4_pool`) is invoked ad-hoc from `bot/engine/verify.rs` as the free-function `liquidity_verifier::*` surface — not a method on a `LiquidityMap`.
...
The f64 Möbius recurrence and the f64-seed-then-refine path are deleted; `mobius_int.rs` carries only the pure-U512 survivors (`IntHopState`, `compute_int_mobius_coefficients`, `int_simulate_path`, `IntMobiusCoefficients`, `SimulationResult`, `MobiusError`). `V2BlockEngine`/`V3BlockEngine`/`V4BlockEngine` survive as sub-state inside `UniswapEngine` with state-only surfaces (no stand-alone `solve_all`/`process_block`/`resolve_path`); the stand-alone `V2ArbEngine`/`V3ArbEngine` PyO3 wrappers and their pumps are deleted. The `mobius_py.rs` PyO3 seam exposes only gen-3-backed classes — `RustPoolCache` (registered-path solving via `exact_mobius_solve`, integer-exact), `RustIntHopState`, `RustArbResult` (no f64 fields).

The Python orchestrator-era solver package (`degenbot/arbitrage/solvers/`: `ArbSolver` / `MobiusSolver` / `PiecewiseMobiusSolver` / Brent / Newton / SolidlyStable) is **kept** for the not-yet-ported paths. It runs entirely on the pure-Python f64 recurrence in `_solver_utils.py` / `_mobius_math.py` — the dependency on the removed Rust f64 PyO3 surface is severed (the Rust fast paths were deleted, the pure-Python fallback that already existed is now the only path). Per the project's Polars-inspired three-layer direction (ADR-005; Rust core + thin Python orchestrator), these Python solvers will be deleted when their Rust-native equivalents under `UniswapArbEngine` arrive.
...
- **PyPoolCache** (Removed under ADR-003): former `parking_lot::Mutex<LruCache<u64, IntHopState>>` solver cache. Live memoization seam is now the per-pool `cached_tick_ranges: parking_lot::Mutex<TickRangeCache>` field on `V3PoolState`/`V4PoolState`, consumed by `build_int_v3_sequence`/`build_int_v4_sequence` and invalidated on every `apply_*`. See ADR-004. (Deleted alongside `RustPoolCache`/`RustIntHopState`/`RustArbResult`.)
- **Dual-Orientation Registration** (Removed under ADR-003): former per-`register_pool()` pattern creating two `IntHopState` entries — forward (reserve0→reserve1) and reverse (reserve1→reserve0). `Bot` now stores one `PoolEntry` per pool; orientation is resolved at solve time (see **Swap Orientation**).
- **V2Snapshot** (Removed under ADR-003): former frozen snapshot of V2 pool reserves at a block, stored in `StateHistory`. Superseded by the delta-based **Reorg Journal**; both `V2Snapshot` and `StateHistory` deleted.
- **StateHistory** (Removed under ADR-003): former `VecDeque<V2Snapshot>` with bounded depth (`push_state`/`discard_before_block`/`restore_before_block`). Superseded by the delta-based **Reorg Journal**.
