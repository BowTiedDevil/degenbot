...
/// Raw `PyErr` type for the gated conversion impls.
///
/// Only available when the `pyo3` feature is enabled. The conversions live here
/// (with the error types) rather than in a downstream crate because the orphan
/// rule forbids `impl ForeignTrait for ForeignType` elsewhere.
#[cfg(feature = "pyo3")]
