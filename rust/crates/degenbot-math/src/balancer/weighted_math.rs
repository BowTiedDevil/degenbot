...
//! percentages). Overflow checks delegate to [`crate::balancer::fixed_point`] (matching
//! Python's `MAX_UINT256`-checked Solidity `SafeMath` reverts).

use alloy::primitives::U256;

use crate::balancer::constants::{PowVersion, ONE};
use crate::balancer::fixed_point::{
...
pub fn calculate_invariant(
    ...
    if invariant.is_zero() {
        return Err(crate::balancer::BalancerMathError::ZeroInvariant);
    }
    Ok(invariant)
}
...
pub fn calc_out_given_in(
    ...
    if amount_in > mul_down(balance_in, MAX_IN_RATIO)? {
        return Err(crate::balancer::BalancerMathError::MaxInRatio);
    }
    let denominator = balance_in
        .checked_add(amount_in)
        .ok_or(crate::balancer::BalancerMathError::AddOverflow)?;
...
pub fn calc_in_given_out(
    ...
    if amount_out > mul_down(balance_out, MAX_OUT_RATIO)? {
        return Err(crate::balancer::BalancerMathError::MaxOutRatio);
    }
    let denominator = balance_out
        .checked_sub(amount_out)
        .ok_or(crate::balancer::BalancerMathError::SubOverflow)?;
