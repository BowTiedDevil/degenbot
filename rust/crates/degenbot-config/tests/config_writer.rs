...
fn load(file: &Path, env: Option<MapEnv>) -> degenbot_config::LoadedConfig {
    let mut loader = BotConfigLoader::new()
        .without_env()
...
fn write_key_persists_and_loads_with_file_provenance() {
    ...
    assert_eq!(loaded.source_of("DEGENBOT_STRATEGY_BACKRUN_ACTIVE"),
...
fn write_key_refuses_an_invalid_value_and_leaves_the_file_untouched() {
    ...
    write_key_with_env(&file, key("strategy.backrun.bribe_bips"), "2000", &empty_env())
        .expect("seed write");
    let before = std::fs::read(&file).expect("read before");
    let err = write_key_with_env(&file, key("strategy.backrun.bribe_bips"), "not-a-number", &empty_env())
        .expect_err("junk must refuse");
    assert!(err.to_string().contains("bribe_bips"), "names the key: {err}");
...
fn write_key_preserves_neighboring_content() {
    ...
    std::fs::write(&file, "# operator note\n[logging]\nlog_stderr = true\n[telemetry]\notel = false\n").expect("seed");
    write_key_with_env(&file, key("strategy.settlement.active"), "true", &empty_env())
        .expect("write");
    let text = std::fs::read_to_string(&file).expect("read back");
    assert!(text.contains("# operator note"), "comment survives: {text}");
    assert!(text.contains("log_stderr = true"), "neighbor key survives: {text}");
    let loaded = load(&file, None);
...
fn remove_key_restores_the_schema_default() {
    ...
    let file = dir.join("config.toml");
    write_key_with_env(&file, key("strategy.backrun.priority_fee_gwei"), "7", &empty_env())
        .expect("write");
...
fn an_env_shadowing_the_key_is_reported() {
    ...
    let outcome = write_key_with_env(&file, key("strategy.backrun.active"), "true", &env)
        .expect("write");
    assert_eq!(
        outcome,
...
fn a_comma_separated_endpoint_list_round_trips() {
    ...
    write_key_with_env(&file, key("strategy.settlement.endpoints"), urls, &empty_env())
        .expect("write");
    let loaded = load(&file, None);
    assert_eq!(
