...
//! pattern: the pyo3-free core (this crate) holds a `BalancerRateProvider`
//! trait, the `degenbot-python` binding supplies a `PyBalancerRateProvider`
//! adapter wrapping a Python callable, and pure-Rust consumers (e.g. a static-rate
//! fallback) supply a pure-Rust `impl` — no Python in the hot path.
//!
//! Mirrors [`crate::tick_fetch::TickWordFetcher`] (the first stored I/O trait
//! object): `Send + Sync` so it can live behind `Option<Arc<dyn …>>` on a
//! `PoolState`, with the Py adapter re-entering via `Python::attach`.
//!
//! ## Why this trait lives in `degenbot-pools` (not the bot)
//!
//! `BalancerStablePoolState` holds an `Option<Arc<dyn
//! BalancerRateProvider>>` field — so the trait type must be visible to this
//! crate. Following the `std::io::Read` precedent (defining an interface
//! pulls no I/O), the trait *definition* + its value-only error type + the
//! pure-Rust `StaticRateProvider` impl live here, while the RPC
//! *implementations* (and the `PyBalancerRateProvider` `PyO3` adapter) live in
