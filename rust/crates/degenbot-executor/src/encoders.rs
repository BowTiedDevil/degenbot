...
//!
//! A pyo3-free *core leaf* porting the tightly-packed command-bytecode
//! encoders from the Python oracle `examples/cmd_stream.py` (constants L60–L113;
//! `_e`/`_address_to_bytes` L122–L150; `AddressTable` L178–L265; `pack_config`/
//! `pack_expected_balance` L137–L184; `enc_preamble`/`enc_set_address`/
//! `enc_set_addresses` L269–L329; the `enc_*` opcode builders L334–L891;
//! `make_pool_key` L898–L918). The authoritative contract layout is
//! `contracts/README.md` "Command-Stream Executor" (opcode | name | encoding |
//! description); Vyper source `~/code/executor/contracts/cmd_executor.vy`.
//!
//! # Scope
//!
...
//!
//! Byte-for-byte parity vs the Python `enc_*` encoders over a fixture corpus
//! generated by `tests/fixtures/generate_encoders_expected.py`; the expected hex
//! is embedded in the `tests` module below. The Python oracle is retained as
//! the parity reference — it is NOT deleted here.
//!
...
/// every uint96 amount field (amounts ≥ this overflow the 12-byte field, as
/// the Python oracle's `_e(amount, 12)` raises `OverflowError`).
...
/// Push an `int16` tick-spacing as two big-endian bytes (the Python oracle
/// encodes `tick_spacing` with `signed=True`). V4 tick spacings are positive in
/// practice; the signed encoding only matters for parity with the oracle.
fn push_i16(out: &mut Vec<u8>, v: i16) {
    out.extend_from_slice(&v.to_be_bytes());
}

/// Push a uint96 amount (≤ `2^96 − 1`) as 12 big-endian bytes; anything larger
/// is rejected, matching the Python oracle's `_e(amount, 12)` `OverflowError`.
...
impl AddressTable {
    ...
    /// [`SENTINEL_NATIVE`] without an explicit [`Self::add`] — match the
    /// Python oracle, which seeds `sentinel_map[NATIVE_ADDRESS] = 0xFF` (and
    /// the same for `ZERO_ADDRESS`, the same address).
    #[must_use]
    pub fn new() -> Self {
        let mut sentinel_map = HashMap::new();
        sentinel_map.insert(NATIVE_ADDRESS, SENTINEL_NATIVE);
        // ZERO_ADDRESS == NATIVE_ADDRESS (both address(0)) — the Python oracle
        // inserts both keys; the second is a no-op overwrite of the same value.
...
/// `Address`'s `Ord`, which compares the big-endian numeric value, matching
/// the Python oracle's `sorted([a, b], key=lambda a: a.lower())`). Mirrors the
...
mod tests {
    ...
    fn uint96_max_is_accepted_overflow_is_rejected() {
        ...
        // 2^96 overflows the 12-byte field (the Python oracle raises).
        assert_eq!(
