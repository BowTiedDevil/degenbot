...
//
// Golden-master expected bytes for `encode_cmd_stream`'s 2-hop paths. The
// Rust `enc_*` primitive sequence is the canonical opcode source (ADR-005);
// these constants record the composed bytestream so a composer change that
// alters output is a visible, reviewable diff. The native-ETH / WETH-bridge
// shapes — where the opcode ORDER is the risk — are covered by `enc_*`-derived
...
use degenbot_executor::composers::{
    self, encode_cmd_stream, encode_execute_call, EncodeOptions, HopInfo, PathInfo, V2HopInfo,
    V3HopInfo, V4HopInfo,
};
