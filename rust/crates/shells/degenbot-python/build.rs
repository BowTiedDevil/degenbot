...
//! configure the build of) the shipped cdylib — this shell, every crate under
//! the explicit `rust/crates/{foundation,engine,integrations,shells,facade}`
//! roles, the workspace manifests, and any `.cargo` config — not just this
//! crate's own files. The Python uv cache-key boundary
...
/// `rust/target` so a `cargo clean` or gc-target sweep can never roll the
/// counter back, and is gitignored so builds never dirty a checkout.
...
fn main() {
    ...
    let counter_path = match std::env::var_os("DEGENBOT_BUILD_NUMBER_FILE") {
        ...
        None => repo_root(&crate_dir).map_or_else(
            || PathBuf::from(COUNTER_NAME),
            |root| root.join(COUNTER_NAME),
        ),
    };
