...
#[pyclass(name = "DatabaseSnapshot", module = "degenbot._ffi.db")]
...
fn build_liquidity_map_py<'py>(
    ...
    for (tick, entry) in &map.tick_data {
        ...
        inner.set_item("liquidity_net", entry.liquidity_net)?;
...
type TickRows = Vec<(PoolKey, HashMap<i32, (U256, i128)>)>;
    ...
    for (key, ticks) in rows {
        ...
        for (tick, (gross, net)) in ticks {
            let pair = PyTuple::new(
                py,
                [u256_to_py(py, &gross)?, net.into_pyobject(py)?.into_any()],
            )?;
