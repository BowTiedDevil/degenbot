...
// ============================================================
// §1.4 idempotent re-run (the pre-existing-desync resilience scenario).
// The user's DB has a pre-epic desync: pools committed by the OLD broken
// per-call-autocommit code, but `last_update_block` never advanced past
// their creation block. When the NEW atomic chunk loop starts from the
// stale `last_update_block + 1`, it re-fetches `PoolCreated` events for
// already-present pools + tries to INSERT them -> `UNIQUE constraint
// failed: pools.address, pools.chain` (the runtime bug).
// The upsert must be idempotent: an INSERT for an already-present pool is
// a no-op (ON CONFLICT DO NOTHING), + the subclass row is skipped too.
// This is NOT the Approach C paper-over (the research doc's rejection
// assumed the stamp would always advance atomically — the pre-existing
// desync breaks that assumption; the new code must be robust to it).
// ============================================================
#[test]
fn re_running_a_chunk_with_already_present_pools_is_idempotent() {
    let (db, spec) = seeded_db_with_v3_exchange();
    let specs = [spec.clone()];
    let pool_addr = Address::from([0x11; 20]);
    let inputs = chunk_inputs_with_v3_pool(spec.id, pool_addr);

    // Chunk [A,B] commits: the pool is now in the DB + the stamp is at B.
    {
        let mut guard = db.lock();
        let tx = guard.transaction().unwrap();
        apply_chunk_writes_on_conn(&tx, CHAIN, &specs, BLOCK_B, &inputs).unwrap();
        tx.commit().unwrap();
    }
    let pools_after_first = {
        let guard = db.lock();
        count_pools(&guard)
    };
    assert_eq!(pools_after_first, 1, "first chunk wrote one pool");

    // Simulate the pre-epic desync: roll the stamp BACK to A-1 (the OLD
    // code's stamp was stale relative to committed pools — the new code
    // did NOT do this, but the user's DB is in this state from the old
    // broken code). The pool rows stay committed.
    {
        let guard = db.lock();
        DegenbotDb::set_exchange_last_update_block_on_conn(
            &guard,
    ...
    // RE-RUN chunk [A,B] (the stale stamp points at A-1, so the chunk loop
    // re-fetches [A,B] + re-discovers the same pool). With a bare INSERT
    // this crashes with UNIQUE constraint failed. With the idempotent
    // ON CONFLICT DO NOTHING upsert, it is a no-op for the pool INSERT.
    let re_run_result = {
        let mut guard = db.lock();
        let tx = guard.transaction().unwrap();
        let r = apply_chunk_writes_on_conn(&tx, CHAIN, &specs, BLOCK_B, &inputs);
        if r.is_ok() {
            tx.commit().unwrap();
        }
        r
    ...
    assert!(
        re_run_result.is_ok(),
        "re-running a chunk with already-present pools must NOT raise a \
         UNIQUE constraint error (idempotent upsert); got: {re_run_result:?}",
    );

    // The pool count is still 1 (no duplicate, no subclass row, no orphan).
    let pools_after_rerun = {
        let guard = db.lock();
        count_pools(&guard)
    };
    assert_eq!(
        pools_after_rerun, 1,
        "idempotent re-run did NOT add a duplicate pool row (still one)",
    );

    // FK integrity: no orphaned subclass row pointing at a non-existent
    // (conflict-skipped) pool id.
    let fk_ok = {
        let guard = db.lock();
        let mut stmt = guard.prepare("PRAGMA foreign_key_check").unwrap();
        let rows = stmt.query_map([], |_| Ok(())).unwrap();
    ...
    assert_eq!(fk_ok, 0, "no orphaned FK rows after the idempotent re-run");
}
