...
//! object (pyo3-free trait in core; `PyCurveDataProvider` adapter in the
...
//!
//! Mirrors [`crate::bot_core::rate_provider::BalancerRateProvider`] and
//! [`crate::bot_core::tick_fetch::TickWordFetcher`]: `Send + Sync` so it can
//! live behind `Option<Arc<dyn …>>` on a `PoolState`, with the Py adapter
