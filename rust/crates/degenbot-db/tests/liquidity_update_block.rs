...
//! (liquidity clock) at `uniswap_v3_pools.liquidity_update_block` rather than the
//! live head, so the registration seed/post-drain verify anchors at the block the DB
//! liquidity map is exact at — not a head block that moved underneath it.
...
fn fetch_liquidity_update_block_reads_v3_pool_row() {
    ...
    let pool: Address = Address::from([0x11u8; 20]);
    {
        let conn = db.lock();
        // FKs off — fixture only needs the two rows the join reads.
        conn.execute_batch("PRAGMA foreign_keys=OFF;").unwrap();
        conn.execute(
            "INSERT INTO pools (id, address, chain, kind, token0_id, token1_id, exchange_id) \
             VALUES (1, ?1, 1, 'uniswap_v3', 1, 2, 1)",
            rusqlite::params![pool.to_checksum(None)],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO uniswap_v3_pools \
             ...
             VALUES (1, 60, 12345, 7, 3000, 3000, 1000000)",
            [],
        )
        .unwrap();
    }
    assert_eq!(
        db.fetch_liquidity_update_block(pool).unwrap(),
        Some(12345),
        "a V3 row's liquidity_update_block is the authoritative liquidity clock"
    );
}
