...
//!
//! The live `[sim-fail] path=26154 … bucket=empty revert=0x` already showed
...
const SOLVE_BLOCK: u64 = 25_700_805;
...
fn build_path(fx: &Fixture, path_id: u64) -> SimulatePath {
    ...
    // The forward-input vector: hop0 = optimal_input, hop i>0 = hop_outputs[i-1].
    // (Consumed by the encoder's CL-hop clamp; equals hop_outputs-shifted here
    // because this fixture's V2/V3/V4 hops are within capacity.)
                ...
                println!(
                    ...
                    if matches_live {
                        "REPRODUCED THE LIVE empty-Halt: bucket=empty, revert=0x matches the live \
                         [sim-fail] path=26154 line exactly. The executor owner gate passed and the \
