...
use degenbot_executor::composers::{encode_cmd_stream, HopInfo};
...
// see them as unused (the types moved out; the lib build no longer names them).
pub(crate) use crate::{
    compute_priority_fee, fits_int128, FailBuckets, SimResult, SimulateContext, SimulatePath,
    EXECUTE_CONFIG, INITIAL_EXECUTE_GAS,
};
...
mod tests {
    ...
    // The sim primitives that moved to `degenbot-evm` — sourced via this crate's
    // root re-export (`BlockPriorityFees`, `INT128_MAX`) + the other leaves the
    // tests construct (`WarmupSlots`, `AlloyProvider`).
    use crate::{BlockPriorityFees, INT128_MAX};
