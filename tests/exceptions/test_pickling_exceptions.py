...
from degenbot.exceptions.pool import HookedPoolResult, IncompleteSwap, StaleRateResult
...
def test_hooked_pool_result_pickling() -> None:
    ...
    original_exception = HookedPoolResult(
    ...
    # Verify the unpickled exception has the same type
    assert type(unpickled_exception) is HookedPoolResult
...
def test_stale_rate_result_pickling() -> None:
    ...
    original_amount_out = 3000
    original_exception = StaleRateResult(
        amount_in=original_amount_in, amount_out=original_amount_out
    )
    ...
    assert type(unpickled_exception) is StaleRateResult
    assert unpickled_exception.amount_in == original_amount_in
    assert unpickled_exception.amount_out == original_amount_out
    assert unpickled_exception.message == (
        "The pool's rate cache is stale; the calculated result may not match on-chain."
    )
