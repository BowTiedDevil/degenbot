...
# TODO: remove these tests after confirming they are unnecessary
class TestInverseProperties:
    """Test that mul and div are approximate inverses for valid operations."""

    @pytest.mark.parametrize("revision", [4, 9])
    @hypothesis.settings(deadline=None)
    @hypothesis.given(
        a=st.integers(min_value=10**18, max_value=10**20),
        b=st.integers(min_value=10**18, max_value=10**20),
    )
    def test_ray_mul_floor_div_ceil_roundtrip(
        self, request: pytest.FixtureRequest, a: int, b: int, revision: int
    ) -> None:
        """
        ray_mul_floor(a, b) then ray_div_ceil(result, b) approximately returns a.

        With large values (> RAY), the roundtrip should give values close to original,
        accounting for rounding in both directions.
        """

        wrapper = request.getfixturevalue(f"token_math_wrapper_rev{revision}")

        try:
            mul_result = wrapper.functions.rayMulFloor(a, b).call()
            div_result = wrapper.functions.rayDivCeil(mul_result, b).call()
        except ContractLogicError:
            hypothesis.assume(condition=False)

        # The roundtrip result should be close to original (within 1%)
        # Due to floor then ceil, we expect: a - epsilon <= div_result <= a + epsilon
        tolerance = max(1, a // 100)  # 1% tolerance or at least 1
        assert abs(div_result - a) <= tolerance, (
            f"Roundtrip error too large: ray_mul_floor({a}, {b}) = {mul_result}, "
            f"ray_div_ceil({mul_result}, {b}) = {div_result}, "
            f"expected within {tolerance} of {a}, got diff {abs(div_result - a)}"
        )
    ...
    def test_ray_div_floor_mul_ceil_roundtrip(
        self, request: pytest.FixtureRequest, a: int, b: int, revision: int
    ) -> None:
        """
        ray_div_floor(a, b) then ray_mul_ceil(result, b) approximately returns a.
        ...
        try:
            div_result = wrapper.functions.rayDivFloor(a, b).call()
            mul_result = wrapper.functions.rayMulCeil(div_result, b).call()
        except ContractLogicError:
            hypothesis.assume(condition=False)

        # The roundtrip result should be close to original (within 1%)
        tolerance = max(1, a // 100)  # 1% tolerance or at least 1
        assert abs(mul_result - a) <= tolerance, (
            f"Roundtrip error too large: ray_div_floor({a}, {b}) = {div_result}, "
            f"ray_mul_ceil({div_result}, {b}) = {mul_result}, "
            f"expected within {tolerance} of {a}, got diff {abs(mul_result - a)}"
        )
