"""Tests for GHO_FLASH_LOAN operation handler."""
...
from degenbot.aave.models import EnrichedGhoDebtBurnEvent
...
class TestGhoFlashLoanHandler:
    """Tests for GhoFlashLoanHandler."""
    ...
    def test_gho_flash_loan_calculates_scaled_amount(
        ...
        operation = _create_mock_operation(OperationType.GHO_FLASH_LOAN, pool_event)
        context = _create_mock_context()

        result = handler.handle(scaled_event, operation, context)

        assert result.raw_amount == raw_amount
        # Scaled amount = raw_amount / index (floor for debt burn)
