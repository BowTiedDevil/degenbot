...
def test_compute_swap_step_exact_amount_in_one_for_zero_that_gets_capped_at_price_target_in():
    ...
    sqrt_q, amount_in, amount_out, fee_amount = compute_swap_step(
        sqrt_ratio_x96_current=price,
        sqrt_ratio_x96_target=price_target,
        liquidity=liquidity,
...
def test_compute_swap_step_amount_out_is_capped_at_the_desired_amount_out():
    sqrt_q, amount_in, amount_out, fee_amount = compute_swap_step(
        sqrt_ratio_x96_current=417332158212080721273783715441582,
        sqrt_ratio_x96_target=1452870262520218020823638996,
...
def test_compute_swap_step_target_price_of1_uses_partial_input_amount():
    sqrt_q, amount_in, amount_out, fee_amount = compute_swap_step(
        sqrt_ratio_x96_current=2,
        sqrt_ratio_x96_target=1,
        liquidity=1,
        amount_remaining=-3915081100057732413702495386755767,
        fee_pips=1,
    )
...
def test_compute_swap_step_not_entire_input_amount_taken_as_fee():
    sqrt_q, amount_in, amount_out, fee_amount = compute_swap_step(
        sqrt_ratio_x96_current=2413,
        sqrt_ratio_x96_target=79887613182836312,
...
def test_compute_swap_step_zero_for_one_handles_intermediate_insufficient_liquidity_in_exact_output_case():
    ...
    fee_pips = 3000
    (sqrt_q, amount_in, amount_out, fee_amount) = compute_swap_step(
        sqrt_ratio_x96_current=sqrt_p,
        sqrt_ratio_x96_target=sqrt_p_target,
        liquidity=liquidity,
