...
def test_to_checksum_address_invalid_byte_length():
    for input_length in list(range(20)) + list(range(21, 32)):
        with pytest.raises(
            ...
            to_checksum_address(b"\x00" * input_length)


def test_to_checksum_address_invalid_hex_string():
    with pytest.raises(ValueError, match="invalid character"):
        to_checksum_address("0x000000000000000000000000000000000000000g")
...
def test_checksum_known_addresses():
    ...
    assert (
        to_checksum_address("0x000000000004444c5dc75cb358380d2e3de08a90")
