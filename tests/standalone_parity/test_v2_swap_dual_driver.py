...
(`PyBot`, the PyO3 binding) produces the **same** `amount_out` as the
...
# `PyBot(chain_id=0)` the (chain, factory) pair is not in the deployments
...
def _register_canonical_v2_pool(py_bot: PyBot) -> int:
...
def test_python_consumer_matches_closed_form() -> None:
    """The PyBot Python driver reproduces the closed-form V2 getAmountOut.
    ...
    """
    py_bot = PyBot()  # chain_id=0 → no CREATE2 verification (ad-hoc path).
    ...
    assert amount_out == _EXPECTED_AMOUNT_OUT, (
        "Python consumer (PyBot FFI seam) must match the closed-form V2 "
...
def test_python_consumer_reverse_matches_closed_form() -> None:
    """The PyBot Python driver reproduces the closed-form V2 getAmountIn.
