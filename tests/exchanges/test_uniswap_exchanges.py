...
def test_deployment_data_has_correct_init_hashes() -> None:
    ...
    assert (
        dep.pool_init_hash == "0x96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f"
    )

    # Uniswap V3 mainnet
    dep = pool_type_registry.get_deployment(1, "0x1F98431c8aD98523631AE4a59f267346ea31F984")
    assert dep is not None
    assert dep.pool_init_hash == (
    ...
    assert (
        dep.pool_init_hash == "0x6ce8eb472fa82df5469c6ab6d485f17c3ad13c8cd7af59b3d4a8026c5ce0f7e2"
    )
