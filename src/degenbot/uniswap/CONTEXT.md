...
| **Pool Init Hash** | The keccak256 hash of pool creation init code, used with CREATE2 to compute pool addresses. Runtime source: the Rust builder resolves the JSON-sourced init_hash at registration and stores it on the pool identity (`V2PoolIdentity`/`V3PoolIdentity`); the `PyLiquidityPool` handle exposes it via `.init_hash`. Non-JSON pools fall back to a Rust `const` (`UNISWAP_V2/V3_MAINNET_INIT_HASH`) — the retired Python `ClassVar`s' documented default (Fork A, P62DKO/NSAZ4X) | Init code hash |
| **Factory Deployment** | Configuration for a specific DEX factory on a chain (factory address, deployer, pool init hash). Single source: `registry/deployments.json`, embedded into the Rust `degenbot-uniswap::deployments` module via `include_str!`; the Python **Pool Type Registry** loads the same JSON for class-resolution. At `register_v2/v3_pool`, the builder resolves the effective deployer (with the `deployer=null -> factory` convention, covering PancakeSwap V3's separate deployer) + init_hash, verifies the CREATE2 address, and stores the verified pair on the pool identity (Fork A) | Exchange deployment |
| **Create2 Deployer** | The address that calls `CREATE2` to deploy a pool; for most DEXes it equals the factory, but PancakeSwap V3 uses a **separate deployer**. Resolved off the handle via `.deployer` (Fork A) — the `dex` getter merges it into the `DexIdentity` preset for the V2 path | deployer, factory deployer |
| **V2PoolDescriptor** | Per-pool registration metadata on `PoolEntry::V2`: `{ variant: DexVariant, stable_swap: bool, fee_denominator: Option<u64> }`. Distinct from the `DexIdentity` **preset** (level-1 DEX data, shared across pools); the descriptor carries the per-pool tag + Camelot stable strategy the builder observed at registration. The Python companion recovers it off the `PyLiquidityPool` handle (Polars `_from_pydf` end state) — `py_pool.dex` merges the protocol-const preset fields (fees/ABI shape) with the JSON-sourced `factory`/`deployer`/`init_hash` stored on the identity (Fork A, NSAZ4X) | Pool descriptor, pool identity |
