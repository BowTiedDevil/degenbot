...
if os.environ.get("DEGENBOT_COVERAGE") and not os.environ.get("PYTEST_VERSION"):
    ...
    def generate_coverage_report() -> None:
        ...
        Each run writes to a uniquely-named data file (e.g.
        .coverage.hostname.pidNNNN.Xrandom) to avoid corrupting a shared
        .coverage file. At exit, all per-run data files are combined with
        any previously-accumulated data in .coverage, then an HTML report
        is generated from the cumulative result.
        """
