...
class Pool:
    ...
    def curve_base_pool(self) -> Pool | None: ...
    def curve_get_dy(
        self,
        i: int,
        j: int,
    ...
    def apply_sync(self, reserve0: int, reserve1: int, block_number: int) -> None: ...
    def apply_balances(self, balances: list[int], block_number: int) -> bool: ...
    def discard_before_block(self, block: int) -> None: ...
    def restore_before_block(self, block: int) -> None: ...
...
class ConcentratedLiquidityView:
    ...
    @property
    def tick_bitmap(self) -> dict[int, tuple[int, int]]: ...
...
class BotIo:
    ...
    # 1-based PoolProbe member code (degenbot.types.pool_type):
    # 1 = V2, 2 = V3, 3 = Balancer weighted, 4 = Balancer stable,
    # 5 = Curve (coins(uint256) answered). No identity answer raises
    # ValueError, never a Curve code; an unrecognized code is seam drift -
    # the Python side raises rather than silently classifying.
    def probe_pool_type(self, address: str, block: int | None = None) -> int: ...
...
class Bot:
    ...
    `Bot` session. The Python ``Bot.__init__`` constructs one; `Pool`/`Erc20Token`
    ...
    def register_v2_pool_test_only(
        self,
        address: str,
        token0: str,
        token1: str,
...
class ArbitrageEngine:
    ...
    engine reads/writes the SAME core that ``Bot``/``Pool``/
    ...
    def resume(self) -> None: ...
    def stop(self) -> None: ...
    # Resolves when the pump task stops (cooperative exit, stream end,
    # abort, or panic) — the awaitable replacement for polling pump_finished.
    def pump_finished_future(self) -> Coroutine[Any, Any, None]: ...
