...
Usage:
    class TokenRegistry(AddressRegistry[Erc20Token]):
        def add(self, token: Erc20Token, chain_id: int) -> None:
            super().add(item=token, chain_id=chain_id, address=token.address)
...
class AddressRegistry[T](AbstractAddressRegistry[T]):
    ...
    Usage:
        registry = AddressRegistry[Erc20Token](name="Token")
        registry.add(item=token, chain_id=chain_id, address=token.address)
        retrieved = registry.get(chain_id=chain_id, address=token_address)
    """
...
class MultiKeyAddressRegistry[T](AbstractAddressRegistry[T]):
    ...
    Usage:
        registry = MultiKeyAddressRegistry[AbstractLiquidityPool](
            address_fields=("pool_manager_address", "pool_id"),
            name="ManagedPool",
        )
        registry.add(
            item=pool,
            chain_id=1,
            pool_manager_address="0xManager...",
            pool_id="0xPoolID...",
        )
