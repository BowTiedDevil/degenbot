...
class BalancerBuilderBase:
    ...
    a BotIo parameter — callers pass the io they receive.
    """
    ...
    @staticmethod
    def _fetch_pool_id(io: BotIo, address: str, block: int) -> bytes:
    ...
    @staticmethod
    def _fetch_swap_fee(io: BotIo, address: str, block: int) -> Fraction:
        return Fraction(io.fetch_balancer_swap_fee(address, block=block), 10**18)

    @staticmethod
    def _fetch_weights(io: BotIo, address: str, block: int) -> list[int]:
        return list(io.fetch_balancer_weights(address, block=block))

    @staticmethod
    def _fetch_amp(io: BotIo, address: str, block: int) -> int:
    ...
    def _detect_pool_type(
        ...
        # (``BotIo.probe_balancer_pool_type``). BotIo is the only executor;
