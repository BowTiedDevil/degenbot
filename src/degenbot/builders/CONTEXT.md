...
**Tick Data Fetcher**: A sync callable `Callable[[int, int], dict[int, tuple[int, int, int]] | None]` created by `make_tick_data_fetcher()` that RETURNS the fetched word's tick data (ADR-005 slice 3b return-data contract — `{tick: (liquidity_gross, liquidity_net, block)}`) rather than writing it back via `update_tick_data`. The Rust `simulate_swap_with_fetch` seam holds the write lock across the fetch call (a write-back fetcher would re-enter it and deadlock); the Python companion's `_apply_fetched_tick_word` helper merges the returned data into the pool state. Returns `None` on pool-not-found / bitmap-RPC failure; `[{}]` for an all-zero bitmap word (known-but-empty). Accepts `io: PoolIO` for I/O. Stored on pool instances for lazy tick population. Async-built pools currently pass `tick_data_fetcher=None` — an async counterpart is not viable because pool objects are synchronous and call the fetcher synchronously during `external_update()`.
