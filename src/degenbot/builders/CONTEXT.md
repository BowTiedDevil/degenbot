...
**PoolIO**: A protocol describing the I/O operations builders need to fetch on-chain data (call, get_provider, etc.). The sync variant is `PoolIOProtocol`; the async variant is `AsyncPoolIOProtocol`. Concrete adapters (`SyncPoolIO`, `AsyncPoolIO`) wrap a `ProviderAdapter` or `AsyncProviderAdapter`.
...
**BuilderContext**: Frozen dataclass passed to sync builders at construction. Carries shared dependencies: `connections`, `db_session`, `erc20_builder`, `default_chain_id`.
_Avoid_: Builder config, builder deps.

**AsyncBuilderContext**: Frozen dataclass passed to async builders at construction. Mirrors `BuilderContext` with async counterparts: `connections: AsyncConnectionManager`, `erc20_builder: AsyncErc20Builder`.
...
**Tick Data Fetcher**: A callable (sync) or async callable (async) that fetches tick/bitmap data for V3/V4 pools. Still coupled to `ProviderAdapter` / `AsyncProviderAdapter` internally — not yet migrated to PoolIO.
...
- **V2-family builders** (V2, Aerodrome V2, Camelot, Async V2) are fully PoolIO-driven — they use `io.call()` / `io.get_provider()` instead of `self._connections.get_provider()`
- **V3/V4/Curve builders** accept `io: PoolIO` on `build()` but still use `self._connections` internally for tick data fetcher callbacks and `CurveDataProviderImpl` — full migration is a follow-up
