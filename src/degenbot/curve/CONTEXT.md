...
| **Per-block caches** | `PerBlockCache` class in `per_block_cache.py` that owns all per-block `BoundedCache` instances for on-chain data. Each has a corresponding `get_cached_*` accessor method implementing the try-cache → call-provider → store → return pattern. Mirror-free design: `get_cached_virtual_price()` resolves its own dependencies by calling `get_cached_base_cache_updated()` and `get_cached_base_virtual_price()` inline, eliminating the former side-effect mirrors (`_base_cache_updated_value`, `_base_virtual_price_value`) and their implicit call-ordering contract. The pool holds `self._cache: PerBlockCache` instead of nine separate fields. `PerBlockCache` handles its own pickle boundary (dropping `_data_provider`). | On-chain cache, cache fields |
