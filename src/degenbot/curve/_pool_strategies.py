...
The address→strategy mapping was derived by transliterating address-based
conditional dispatch into enum-based dispatch. That transliteration
preserved the *dispatch path* but did NOT verify each address against
the on-chain contract source.
...
1. **Wrong enum value.** An address was grouped with others that share
   ...
   Example: 0xA5407eAE has USE_LENDING = [False, False, False, False] —
   it's not a cToken pool, but the dispatch path works because both
   paths return PRECISION_MUL * LENDING_PRECISION when no lending is active.
...
_POOL_STRATEGIES: dict[ChecksumAddress, PoolStrategies] = {
    ...
    # ⚠️  0xA5407eAE has USE_LENDING=[F,F,F,F] (no lending tokens).
    # Its _stored_rates() returns PRECISION_MUL * LENDING_PRECISION, identical
    # to rate_multipliers. LendingRateStyle remains NONE.
