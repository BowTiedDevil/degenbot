...
def to_0x_hex(data: bytes | bytearray | str) -> str:
    ...
    Returns:
        The hex string with a ``0x`` prefix (``"0x"`` for empty input).
...
def to_bytes(data: BytesLikeSource) -> bytes:
    r"""Decode a value to ``bytes`` following the old wrapper's matrix.
    ...
    - ``str`` -- ``bytes.fromhex(s)`` after skipping an optional ``0x``
      prefix, with odd-length hex left-padded (upstream rule); ASCII non-hex
      chars raise ``binascii.Error`` (a ``ValueError`` subclass) and
      non-ASCII characters propagate the ``UnicodeEncodeError`` (upstream's
      ``except`` clause catches ``UnicodeDecodeError``, which
      ``.encode()`` never raises);
    ...
    Raises:
        TypeError: If *data* is not one of the accepted container types.
        ValueError: If *data* is a negative ``int`` or a string containing
            non-hex characters (``binascii.Error``).
    ...
    if isinstance(data, int):
        if data < 0:
            msg = f"Cannot convert negative integer {data} to bytes"
    ...
    msg = f"Cannot convert {data!r} of type {type(data)} to bytes"
    raise TypeError(msg)
