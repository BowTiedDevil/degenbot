...
| **Engine-Facing Hop Descriptor** | The `V2HopInfo` / `V3HopInfo` / `V4HopInfo` / `SolidlyHopInfo` / `HopInfo` / `PathInfo` / `build_hops_from_pools` family in `degenbot.arbitrage.hop_info` — distinct from the *solver's* `HopType`/`BoundedProductHop` shape. Frozen dataclasses reading only pool attributes, used by `EngineRegistry.register_path` to hand hops to the Rust engine and read back by deployment-side `encode_cmd_stream`. `SolidlyHopInfo` (the Aerodrome stable/volatile + Camelot `stable_swap` descriptor) carries the per-direction fee as a `Fraction`, the `stable` flag, and the kebab-case DexVariant that selects the solidly-math leaf; `PathInfo.path_type` emits `Solidly` for these hops. The PyO3 `register_path` takes only `(pool_id, zero_for_one)` — the Rust `resolve_path` / `derive_hop_type` classifies Solidly from the `BotState` identity (no SolidlyHopInfo round-trip through FFI). | Hop info, hop struct |
...
| **Solidly Solver** *(removed)* | The Solidly stable invariant solve (`x³y + xy³ ≥ k`) is Rust-owned: the engine's `solve_solidly_path_int` (Möbius precheck + 25-iter golden-section over the `degenbot-solidly-math` integer leaf — `get_y_solidly` / `get_y_camelot`, EVM-exact against on-chain Solidity) is the production solve surface. The Python `SolidlyStableSolver` parity oracle (same algorithm, same integer leaf) is **retired** per the §4.3 oracle-retirement protocol — it cross-validated the engine in `test_engine_vs_solidly_parity` alongside `BrentSolver` (the looser f64-outer oracle, still kept); parity was proven and the oracle was deleted together with its tests, its `solvers/__init__.py` re-export, and its dedicated `test_solidly_stable_solver.py` suite. The generic mixed-path simulators (`_simulate_mixed_path` / `_simulate_mixed_path_int`) moved to `_solver_utils.py` (reused by the kept Curve tests). Not on the production solve path. | Solidly, solidly-stable |
