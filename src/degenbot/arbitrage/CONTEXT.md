...
| **Solidly Solver** *(parity oracle only)* | The Solidly stable invariant solve (`x³y + xy³ ≥ k`) is Rust-owned: the engine's `solve_solidly_path_int` (Möbius precheck + 25-iter golden-section over the `degenbot-solidly-math` integer leaf — `get_y_solidly` / `get_y_camelot`, EVM-exact against on-chain Solidity) is the production solve surface. `SolidlyStableSolver` is kept as the Python **parity oracle** (same algorithm, same integer leaf) — cross-validated in `test_engine_vs_solidly_parity` alongside `BrentSolver` (the looser f64-outer oracle). Not on the production solve path. | Solidly, solidly-stable |
