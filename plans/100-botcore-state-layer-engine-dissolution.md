...
[x] Slice 2b (part 2): V3 single-pool calc — **done**
    - `v3_simulate_swap` (port of `v3_simulator.calculate_swap` / V3's `_calculate_swap`) added to `bot_core/v3_state.rs`: walks ticks via `gen_ticks`, runs `compute_swap_step_v3` per tick range, crosses `liquidity_net` at initialized boundaries (zfo subtracts net, ofz adds net), derives `amount0`/`amount1` per the V3 Swap callback convention. Returns `Option<V3SwapOutcome>` (None on zero amount / math failure / invariant violation — callers fall back to U256::ZERO).
    - `BotCore::calculate_tokens_out` (exact-in, positive `amount_specified`): V3 arm returns `amount1` for zfo / `amount0` for ofz; `calculate_tokens_in` (exact-out, negative `amount_specified`): V3 arm returns `amount0` (zfo) / `amount1` (ofz). The V3 "Slice 7 not implemented" `U256::ZERO` stubs are gone.
    - Tests cross-check the simulator against `compute_swap_step_v3` directly (zero hand-math — the tested swap-step fn is the oracle for the no-crossing case) for both zfo + ofz, plus zero-amount-returns-None and output-monotonic-in-input. 487 Rust + 352 Python + clippy clean.
[x] Slice 3: V4 consolidation + orphan-solve reconciliation + V4 single-pool calc — **done** (deferred the standalone `LiquidityMap` generic extraction; the S1 inline-`PoolEntry`+`BotCore::apply_*` pattern suited V3+V4 without forcing a generic. Deferred to a future refactor once Curve ports.)
    - New `bot_core/v4_state.rs` submodule (merged `V4PoolState` + `V4PoolKey` + `RegisterV4PoolParams` + `V4SwapUpdate` + `AMOUNT_MODIFYING_HOOK_MASK`/`V4_DYNAMIC_FEE_FLAG` + buffer type + `build_int_v4_sequence` + `v4_simulate_swap`). The `PoolTickCoverage` enum now lives in `bot_core::v3_state` and is shared.
    - `PoolEntry::V4` added; `BotCore` gains the V4 live-path surface mirroring V3: `register_v4_pool` (inline hook filter), `apply_v4_swap` (journals scalars + per-tick priors), `apply_v4_liquidity_update`, `buffer_backfill_v4_liquidity_update`, `apply_backfill_buffer_v4` / `apply_pump_buffer_v4` (staged drain), `expire_v4_buffered`, `set_v4_buffer_max_age`, `flush_v4_buffer`, `get_v4_pool`, `v4_pool_id_by_key`, `v4_pool_count`, `v4_registered_pool_managers`, `v4_pools_snapshot`, `sync_v4_pool_state`, `v4_journal_len` / `v4_discard_before_block` / `v4_restore_before_block`. A `v4_buffer` field + `v4_pool_ids: HashMap<(Address, PoolId), u64>` registry. `restore_all_pools_before_block` extended to V4. `calculate_tokens_out`/`in` now serve V4 via `v4_simulate_swap` (V4 sign convention: exact-input is `amountSpecified < 0`).
    - **Orphan-solve retired**: V4BlockEngine's parallel path registry (`register_path`, `solve_all`, `rebuild_and_solve_affected`, `resolve_path`, `rebuild_and_solve`, `paths`, `results`, `ResolvedV4Path`, `V4Path`, `V4PoolRef`) was dead code — `v4_engine.register_path` had zero callers; the unified `UniswapEngine.register_path`/`solve_path` handles V4 paths via `HopType::V4`. The orphan `rebuild_and_solve` call in `process_backfill_logs` is removed; the unified engine re-derives on the next `solve_dirty`.
