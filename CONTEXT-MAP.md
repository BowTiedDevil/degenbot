...
- [Rust Extension](rust/CONTEXT.md) — PyO3-wrapped ABI encode/decode, GIL discipline, subscription double-buffer, two-level type intern, f64↔U256 conversion, UniswapArbEngine with V2+V3+V4 composition, **BotCore as the single state owner (ADR-003, implemented Plan 100)** with V2/V3/V4 `PoolEntry` variants, {Removed-Flag} reorg detection feeding the {Reorg Journal} (default 32 blocks), eager per-log processing (`apply_log` + `solve_dirty` coalesced at pump loop top, with the pump-mutates-core, engine-reads-by-reference read/write asymmetry per the {Pool's authority over its own math} rule), shared tick update helpers (`update_tick_liquidity` and `apply_liquidity_to_tick_range` in `tick_bitmap.rs`, matching Solidity's `Tick.update()`), V4 hook filtering (0xCC mask) and dynamic fee exclusion (0x100000), V4 amountSpecified sign convention (negative = exact-input, opposite to V3), unbounded result channel with incremental `ResultBatch` diffs. The `V2BlockEngine`/`V3BlockEngine`/`V4BlockEngine` are **dissolved** (state lives in `BotCore`); the legacy `RustPoolCache` + `ArbPoolCacheAdapter` mirror is **deleted** (ADR-003 Slice 4 Option D — delete, not migrate).
