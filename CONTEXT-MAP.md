...
- [Rust Extension](rust/CONTEXT.md) — PyO3-wrapped ABI encode/decode, GIL discipline, subscription double-buffer, two-level type intern, f64↔U256 conversion, UniswapArbEngine with V2+V3+V4 composition, **Bot as the single state owner (ADR-003, implemented Plan 100)** with V2/V3/V4 `PoolEntry` variants, {Removed-Flag} reorg detection feeding the {Reorg Journal} (default 32 blocks), eager per-log processing (`apply_log` + `solve_dirty` coalesced at pump loop top, with the pump-mutates-core, engine-reads-by-reference read/write asymmetry per the {Pool's authority over its own math} rule), shared tick update helpers (`update_tick_liquidity` and `apply_liquidity_to_tick_range` in `tick_bitmap.rs`, matching Solidity's `Tick.update()`), V4 hook filtering (0xCC mask) and dynamic fee exclusion (0x100000), V4 amountSpecified sign convention (negative = exact-input, opposite to V3), unbounded result channel with incremental `ResultBatch` diffs. The `V2BlockEngine`/`V3BlockEngine`/`V4BlockEngine` are **dissolved** (state lives in `Bot`); the legacy `RustPoolCache` + `ArbPoolCacheAdapter` mirror is **deleted** (ADR-003 Slice 4 Option D — delete, not migrate). The FFI topology for the `Bot`/`PyBot`/`PyLiquidityPool`/`PyErc20Token` family is **ADR-005** (Polars-Inspired Three-Layer Architecture).
...
- **V2 N-hop encoding** (`_encode_cmd_v2_n_hop` in `eth_backrun_helpers.py`): Approach 2 (V2_SWAP_COMPACT flash + chained V2_SWAP_CALC). Pool A sends forward token optimistically; callback transfers to pool B; V2_SWAP_CALC reads excess balance at each subsequent pool. **Address table index bug** (fixed): list comprehension `for h in hops` referenced `hop.pool_address` from outer scope instead of `h.pool_address` — all pools resolved to the last hop's deduplicated index, causing the executor to call the wrong pool for every V2_SWAP_CALC. Symptom: 100% V2-V2-V2 simulation failure, `V2_SWAP_CALC: no excess balance` revert with zero sub-calls in debug_traceCall.
...
- The **Rust extension** provides PyO3-wrapped ABI encode/decode (`CachedAbiTypes` two-level intern: string `Arc<str>` interner + value `Arc<CachedAbiTypes>` return), subscription double-buffer (`drain_raw()` for pure-Rust GIL-free accumulation, `drain_buffer()` for Python), and Möbius solver cache (`PyPoolCache` with `parking_lot::Mutex<LruCache<u64, IntHopState>>`, 10K cap, pre-converted U512 fields). GIL discipline: hold for sub-μs compute (tick math, address utils), release for I/O (`py.detach()` before `block_on()`); all `Python::attach()` call sites have `// SAFETY` comments. `f64_to_u256` uses iterative 4-limb decomposition (the previous 2-limb version silently failed for values > 128 bits). `auto-initialize` is the default Cargo feature. **UniswapArbEngine** composes `V2BlockEngine` + `V3BlockEngine` + `V4BlockEngine` — V4 pools are concentrated liquidity identified by `(pool_manager, pool_id)` and are solved with the same `int_solve_v3_v3`/`exact_solve_mixed_v2_v3_sequence` as V3. **Eager per-log processing**: the `UniswapEnginePump` applies each WS log to engine state immediately via `apply_log` (zero-latency), with solver runs deferred and coalesced at the top of the pump loop — multiple logs between `await` points are naturally batched into a single `solve_dirty` call, avoiding redundant O(total_paths) work for hot pools. Sub-engine `apply_*` methods return affected pool keys for the reverse index. Block boundaries detected by `log.block_number > current_block` (primary) or `header.number > current_block` (empty-block fallback); headers provide metadata and trigger empty-block batches. **Result channel** is unbounded `mpsc` — every incremental `ResultBatch` diff (`fresh`/`updated`/`expired`/`removed`) is delivered to Python, no silent drops. V3 `process_block()` now decodes Swap, Mint, and Burn events; `apply_liquidity_update()` mutates tick_data matching Solidity's `Tick.update()` — both lower and upper tick receive `gross += delta`, while `net` flips sign for the upper tick. V4 pools with amount-modifying hooks (`AMOUNT_MODIFYING_HOOK_MASK = 0xCC`) or dynamic fees (`V4_DYNAMIC_FEE_FLAG = 0x100000`) are rejected in Python before `register_v4_pool` — the Rust engine is permissive. V4 `amountSpecified` uses negative values for exact-input (opposite convention to V3). **`backfill_from_snapshot()`** bridges the cold-start gap: after `subscribe()` and before `resume()`, fetches Mint/Burn/ModifyLiquidity events from `snapshot_block + 1` to `first_ws_block - 1` via paginated `eth_getLogs`, applying them via `process_backfill_logs()`. For unregistered pools (all of them at this point), liquidity events are routed to `backfill_event_buffer` (never expired) via `buffer_backfill_liquidity_update()`. WS pump events for unregistered pools go to `pump_event_buffer` (expired normally). `register_pool()` no longer applies buffers inline — the caller sequences `apply_backfill_buffer()` then `apply_pump_buffer()` under the engine lock, enabling race-free two-phase verification: raw tick_data vs on-chain at snapshot block, post-backfill state vs on-chain at backfill block.
