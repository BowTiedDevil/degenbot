...
- ✅ "The `pool_type_registry` maps SushiSwap's factory to a `UniswapV2Pool` registration carrying the `sushiswap-v2` `DexIdentity` preset + `variant="sushiswap"` (post slice-7 collapse — the per-DEX `SushiswapV2Pool` subclass is deleted)"
